...
//     session_destroy();
//     header("Location: ../?page=companyAnnoun");
//     exit();
// }
...
// รายการ pagintion ทั้งหมด 
$count_sql = "SELECT COUNT(*) as total FROM documents_announ WHERE 1 $year_filter";
$count_result = mysqli_query($conn, $count_sql);
// $total_records = mysqli_fetch_assoc($count_result)['total']; เนื่องจากใช้คำสั่งนี้กับ version เก่าไม่ได้
$total_records = mysqli_fetch_assoc($count_result);
        ...
        <div class="header-content">
            ...
            <div class="user-info">
                <div class="user-details">
                    ...
                    <button type="button" class="btn"><div class="changpassword" onclick="openPasswordModal()"> Change Password</div> </button>
        ...
        <div class="container" data-aos="fade-up" data-aos-delay="100">
            <div class="row shadow-lg p-3 m-5 bg-body rounded" >
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center;">
                                        <i class="bi bi-pencil-square" onclick="editDocument(<?php echo $row['DocID']; ?>, 
                                                '<?php echo addslashes($row['Number']); ?>', '<?php echo $row['Year']; ?>', 
                                                '<?php echo addslashes($row['Title']); ?>', '<?php echo $row['Doc_date']; ?>', 
                                                '<?php echo $row['TH_Path']; ?>', '<?php echo $row['EN_Path']; ?>', 
                                                '<?php echo $row['Status']; ?>')">
                                        </i>
                                    </td>
                                </tr>
                ...
                <div class="pagination mt-3">
                    ...
                    ?>
                    <!-- ปุ่ม Before -->
                    <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => $prev_page])); ?>"
                        class="btn btn-sm btn-secondary me-2">Before</a>
                    <!-- แสดงหน้าปัจจุบัน -->
                    <span class="page-number">Page <?= $page ?> of <?= $total_pages ?></span>
                    <!-- ปุ่ม Next -->
                    <a href="?<?php echo http_build_query(array_merge($_GET, ['page' => $next_page])); ?>"
